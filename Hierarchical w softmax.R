...
jdat <- list(
  N = length(unique(df_long$Year)),           # number of years
  S = length(unique(df_long$sero_id)),        # number of serotypes
  counts = matrix(df_long$cases,
...
# 7. Run model
mod <- jags.model(textConnection(jcode3), data = jdat, n.chains = 2)
...
year_map <- data.frame(year_id = year_seq, Year = sort(unique(df_long$Year)))
pred_grid <- left_join(pred_grid, year_map, by = c("year" = "year_id"))

# 11. Add serotype labels back to df_long for plotting
...
carriage_plot <- ggplot(df_long, aes(x = Year, group = Serotype, color = Serotype)) +
  geom_point(aes(y = Percent), alpha = 0.6) +
  geom_line(aes(y = expected_cases), size = 1) +
  scale_x_continuous(
    breaks = seq(floor(min(df_long$Year)), ceiling(max(df_long$Year)), by = 2),
    limits = c(2009, 2021)
  ) +
...
carriage_plot <- ggplot(df_long, aes(x = Year, group = Serotype, color = Serotype)) +
  ...
  labs(
    ...
    subtitle = expression("For year " * t * " and serotype group " * s * 
                            ":  " * 
...
ggsave("Carriage_Plot_HM.jpeg", plot=carriage_plot, width=8, height=5)
