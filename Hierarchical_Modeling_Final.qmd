...
#plot
model_plot <- ggplot(carriage_serotype_year_counts, aes(x = Year)) +
...
ggsave("model_plot.jpeg", model_plot, width=10, height=6)
...
# Assign numeric IDs for JAGS
carriage_serotype_proportions <- carriage_serotype_proportions %>%
  mutate(sero_id = as.integer(factor(Serotype)), # assigning IDs for serotypes
         year_id = as.integer(factor(Year))) # assigning IDs for the year

# Create JAGS data list
jdat <- list(
  N = nrow(carriage_serotype_proportions), # number of rows = number of cases
  cases = carriage_serotype_proportions$Percent, # cases = cases
  sero_id = carriage_serotype_proportions$sero_id, # serotype IDs
  year_id = carriage_serotype_proportions$year_id, # year IDs
  n_sero = length(unique(carriage_serotype_proportions$sero_id)), # number of unique serotypes
  n_year = length(unique(carriage_serotype_proportions$year_id))) # number of years
...
ggplot(carriage_serotype_proportions, aes(x = Year, y = Serotype, fill = Percent)) +
  geom_tile(color = "white") +
...
#model version
mean_year <- (max(carriage_serotype_proportions$year_id) + 1) / 2

# Create a data frame with expected log lambda for each serotype-year
carriage_serotype_proportions$expected_log_lambda <- mu_means[carriage_serotype_proportions$sero_id] + 
  beta_means[carriage_serotype_proportions$sero_id] * (carriage_serotype_proportions$year_id - mean_year)

# Convert to expected cases (lambda)
carriage_serotype_proportions$expected_cases <- exp(carriage_serotype_proportions$expected_log_lambda)
#plot
ggplot(carriage_serotype_proportions, aes(x = Year)) +
...
carriage_serotype_proportions$Year = as.numeric(carriage_serotype_proportions$Year)

ggplot(carriage_serotype_proportions, aes(x = as.integer(Year), y = Percent, group = Serotype, color = Serotype)) +
  geom_point(alpha = 0.6) +
  geom_line(aes(y = expected_cases, group = Serotype), size = 1) +
  scale_color_manual(values = custom_colors) +
  labs(y = "Percent of Cases Per Year", x = "Year", title = "Carriage",
       color = "Serotype Group") +
  theme_linedraw() +
  ylim(0, 80) +
  scale_x_continuous(breaks=seq(floor(min(carriage_serotype_proportions$Year)), ceiling(max(carriage_serotype_proportions$Year)), by = 2))+
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        axis.title = element_text(size=14),
        axis.text = element_text(size=14),
        plot.subtitle = element_text(size=14),
        plot.title = element_text(size=15, hjust = 0.5),
        legend.position = "none") -> carriage_plot
carriage_plot
```
...
disease_serotype_proportions$Year = as.numeric(disease_serotype_proportions$Year)

ggplot(disease_serotype_proportions, aes(x = as.integer(Year), y = Percent, group = Serotype, color = Serotype)) +
  geom_point(alpha = 0.6) +
  geom_line(aes(y = expected_cases, group = Serotype), size = 1) +
  scale_color_manual(values = custom_colors) +
  labs(y = "Percent of Cases Per Year", x = "Year", title = "Disease",
       color = "Serotype Group") +
  theme_linedraw() +
  ylim(0, 80) +
  xlim(2009, 2021) +
  scale_x_continuous(breaks=seq(floor(min(disease_serotype_proportions$Year)), ceiling(max(disease_serotype_proportions$Year)), by = 2))+
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        axis.title = element_text(size=14),
        axis.title.y = element_blank(),
        axis.text = element_text(size=14),
        plot.title = element_text(size=15, hjust=0.5),
        legend.position="none") -> disease_plot
disease_plot
...
ggplot(disease_serotype_proportions, aes(x = as.integer(Year), y = Percent, group = Serotype, color = Serotype)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        ...
        legend.title = element_text(size=14)) -> legend_plot
legend_plot
```

```{r plots together}
library(patchwork)
disease_plot <- disease_plot + theme(legend.position = "right",
                                     legend.title = element_text(size=14),
                                     legend.text = element_text(size=12))
# Combine with shared legend and title
PLOT8 <- (carriage_plot | disease_plot) +
  plot_layout(guides = "collect") +
  plot_annotation(
    title = "Hierarchical Modeling - Percent of Cases from Each Serotype Group in Each Year",
    subtitle = expression("Cases ~ " * beta(Serotype~Group[i]) +
                          mu(Serotype~Group[i] * (Year~Index[j] - Mean~Year)))
  ) &
  theme(
    plot.title = element_text(hjust = 0.3, size = 18, face = "bold"),
    plot.subtitle = element_text(hjust = 0.3, size = 12, color = "#6A0DAD")
  )

PLOT8
ggsave("plot8.png", plot=PLOT8, width=12, height=7)
```
