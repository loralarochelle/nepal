...
All data together: (not super useful, but a good start)

```{r hierarchical modeling all}
library(tidyverse)
...
carriage_serotype_year_counts <- carriage_serotype_year_counts %>% 
  filter(Year >= 2009 & Year != 2014 & Year != 2021)
...
>>>>>>> 99a7dd0f1b1bd9ec86b6985005be900082c526c4
...
custom_colors <- c(
  "PCV10serotypes" = "#00356B",   # Yale Blue
  "PCV13serotypes" = "#6A0DAD",   # Purple
  "PCV15serotypes" = "#008080",   # Teal
  "PCV20serotypes" = "#E69F00",   # Gold
  "Other" = "#999999"    # Grey fallback
)
...
ggplot(disease_serotype_proportions, aes(x = as.integer(Year), y = Percent, group = Serotype, color = Serotype)) +
  ...
  geom_line(aes(y = expected_cases, group = Serotype)) +
  scale_color_manual(values = custom_colors) +
  labs(y = "Percent of Cases", x = "Year", title = "Percent of Total Pneumococcal Disease Infections Per Year From Each Serotype Group", color = "Serotype Group") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
