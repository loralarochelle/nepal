...
ggplot(nepal_serotype_counts, aes(x = Year, y = count, color = Serotype_group)) +
  ...
  labs(title = "Pneumococcal Disease Prevalence in Nepal\nby Serotype Group Over Time", y = "Case Count", x = "Year", color = "Serotype Group") +
  theme_bw() +
  theme(plot.title = element_text(face = "bold", size = 14))

# Set proper factor levels for ordered PCV groups
pcv_levels <- c("PCV10", "PCV13 (non-PCV10)", "PCV15 (non_PCV13)", "PCV20")
nepal_serotype_props <- nepal_serotype_props %>%
  mutate(Serotype_group = factor(Serotype_group, levels = c(pcv_levels, "Other")))

# Compute cumulative proportions only for PCV groups
nepal_cumulative_props <- nepal_serotype_props %>%
  arrange(Year, Disease_Status, Serotype_group) %>%
  group_by(Year, Disease_Status) %>%
  mutate(
    cum_prop = if_else(
      Serotype_group %in% pcv_levels,
      cumsum(if_else(Serotype_group %in% pcv_levels, prop, 0)),
      prop)) %>%
  ungroup()
...
# CUM SUM ONE
ggplot(nepal_cumulative_props, aes(x = Year, y = cum_prop, color = Serotype_group)) +
  geom_line() +
  geom_point() +
  facet_grid(Disease_Status ~ .) +
  scale_color_manual(values = custom_colors) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(title = "Proportion of Pneumococcal Infecton \nSerotype Groups by Year in Nepal",
    y = "Proportion of Cases",
    x = "Year",
    color = "Serotype Group") +
  theme_linedraw() +
  theme(plot.title = element_text(face = "bold", size = 14),
    strip.text = element_text(size = 12),
    axis.title = element_text(size = 11)) -> cumprops
ggsave("propsplot.png", plot=props, width=7, height=6)
ggsave("cumpropsplot.png", plot=cumprops, width=7, height=6)
```
